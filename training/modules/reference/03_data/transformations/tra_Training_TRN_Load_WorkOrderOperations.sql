...
-- specific, silent way. Chapter 05 section 5.6 walks through each failure in turn.
...
with ops as (
  select
    ...
    -- into real NULLs so the filter below can catch them.  (section 5.6.3)
    ...
    -- external IDs stop matching the source system.  (section 5.6.4)
    concat(
    ...
    -- no warning, just a smaller row count than you expected.  (section 5.6.1)
...
deduped as (
  ...
  -- the rows genuinely differ. Pick a winner explicitly.  (section 5.6.2)
  select
    *,
...
select
  ...
  -- deliberate: Chapter 16 teaches you to find the ones that dangle.  (section 16.2)
