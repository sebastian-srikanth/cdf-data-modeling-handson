...
def _ensure_dms_cad_model(client, model_name: str, space: str) -> SimpleNamespace:
    ...
    while True:
        ...
        if not cursor:
            break
    # PROJECT-DEPENDENT payload -- see the [LIMITS] box in §9.1.
...
- **A list endpoint returning `[]` is not proof of absence.** You have now met this three
  ...
  ([Chapter 08](08-diagram-annotation.md) §8.7), `views.list(space=[a, b])`
  ([Chapter 13](13-querying-the-graph.md) §13.11), and this. When an empty result
